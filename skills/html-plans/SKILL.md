...
- External fonts and stylesheets are blocked (self-contained requirement). Theme with **system font stacks** — e.g. `Palatino, Georgia, serif`, `Futura, "Avenir Next", sans-serif`, `Menlo, monospace`, `"Marker Felt", fantasy` — or embed a small woff2 as a data URI if a font is essential.
...
The template includes copy-paste patterns for all of these — pure HTML/CSS, no JS:
...
Postplan serves plans with `script-src 'none'` — mermaid **cannot render in the browser**. Render at authoring time and inline the SVG:
...
- **Always keep the mermaid source** in a `<details class="mermaid-source">` immediately after the figure, in a `<pre>` block — that's how read-plans and future edits recover the diagram.
...
- Edit the same file and re-upload — URL stays stable, history kept (`/v/<n>/raw`).
