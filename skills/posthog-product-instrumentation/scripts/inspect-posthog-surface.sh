...
usage() {
  cat <<'EOF'
Usage:
  inspect-posthog-surface.sh --repo /path/to/project [--format text|json]
...
usage_files_json="$(
  rg -l --hidden --glob '!node_modules' --glob '!\.git' \
    'posthog|capture\(|identify\(|group\(|useFeatureFlag|featureFlags|PostHogProvider' \
    "$repo" 2>/dev/null \
    | sed "s|$repo/||" | sort -u | jq -R . | jq -s 'map(select(length > 0))'
)"
...
emit_text() {
  printf 'repo\t%s\n' "$repo"
  printf 'posthog_package\t%s\n' "$(has_package 'posthog-js' && echo present || has_package 'posthog-node' && echo present || echo missing)"
...
emit_json() {
  jq -cn \
    --arg repo "$repo" \
    --argjson posthogPackage "$(has_package 'posthog-js' && echo true || has_package 'posthog-node' && echo true || echo false)" \
    --argjson envFiles "$env_files_json" \
    --argjson usageFiles "$usage_files_json" \
    '{repo: $repo, posthogPackage: $posthogPackage, envFiles: $envFiles, usageFiles: $usageFiles}'
}
